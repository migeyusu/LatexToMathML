...
namespace LatexToMathML
{
    ...
    public sealed class LatexMathToMathMLConverter
    {
        ...
        public LatexMathToMathMLConverter(ILogger? logger = null)
        {
            this._logger = logger;
        ...
        private void ThreadConvert(object obj)
        {
            try
            {
                if (!(obj is string latexExpression))
                {
                    throw new ArgumentException("The parameter must be a string instance.");
                }

                var parser = new LatexParser(latexExpression, this);
                var root = parser.Root;
                CallEventHandler(AfterDocumentWasParsed);
                var linkToMmlddt = "<!DOCTYPE math SYSTEM 'http://www.w3.org/Math/DTD/mathml1/mathml.dtd'>";
                var sb = new StringBuilder(linkToMmlddt);
                Output = root.Convert();
                ...
                if (ValidateResult)
                {
                    ...
                    settings.ValidationEventHandler += (s, e) =>
                    {
                        ...
                        throw e.Exception;
#else
                        _logger?.LogDebug("DTD Validator - " + e.Message, e.Exception);
        ...
        public void Convert(String latexExpression)
        {
            ...
            {
                var thread = new Thread(new ParameterizedThreadStart(ThreadConvert));
                thread.Start(latexExpression);
                var checkPoint = DateTime.Now;
                thread.Join(120000);
                if ((DateTime.Now - checkPoint).Seconds > 119)
                {
                    _logger?.LogError("It took more than 2 minutes to convert the document");
                }
            }
            else
            {
                var thread = new BackgroundWorker();
                thread.DoWork += (s, e) => ThreadConvert(latexExpression);
...
#pragma warning restore 168
                {
                    ...
                    throw;
#else
                    _logger?.LogDebug("A user exception occured when " +
