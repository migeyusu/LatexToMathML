...
namespace LatexToMathML
{
    ...
    public class LatexExpression
    {
        ...
        /// <param name="indexInParentChild">Index in the parent child outline.</param>
        /// <param name="customization">The LatexMathToMathMLConverter class instance to customize the conversion result.</param>
        public LatexExpression(string name, LatexExpression parent, int parentChildNumber, int indexInParentChild,
            LatexMathToMathMLConverter customization)
        {
            Name = name;
            Customization = customization;
            ExprType = ExpressionType.Block;
        ...
        private void ParseOptions(string? options)
        {
            ...
            var splitedOptions = options.Split(',');
            Options = new ExpressionOptions
            {
                AsKeyValue = new Dictionary<string, string>(splitedOptions.Length)
            };
            ...
            string beginning, TextReader? rdr, out string? end, ref bool whitespaceBefore)
        {
            ...
            switch (str[0])
            {
                ...
                default:
                    ...
                    return new LatexExpression(parent, parentChildNumber, ref indexInParentChild, ref verbatimMode,
                        ExpressionType.PlainText, mathMode, customization,
                        (whitespaceBefore ? " " : "") + SmartTextTrim(name), null, null);
        ...
        /// </summary>
        private static readonly Dictionary<ExpressionType, BaseConverter> Converters =
            new Dictionary<ExpressionType, BaseConverter>
            {
